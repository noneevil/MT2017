...
namespace CommonUtils.IHttpModule
{
    public class ImageHandler
    {
        ...
        public static void WaterText(String filePath, String savePath, String watermarkText)
        {
            using (Image initImage = Image.FromFile(filePath))
            {
                using (Graphics gWater = Graphics.FromImage(initImage))
                {
                    Font fontWater = new Font("黑体", 10);
        ...
        /// 图片加水印文字
        /// </summary>
        /// <param name="filePath">旧图片地址</param>
        /// <param name="text">水印文字</param>
        /// <param name="savePath">新图片地址</param>
        /// <param name="Alpha">透明度</param>
        /// <param name="fontSize">字体大小</param>
        public static void AddWaterText(String filePath, String text, String savePath, double Alpha, int fontSize)
        {
            using (Image imgPhoto = Image.FromFile(filePath))
            {
                int imgPhotoWidth = imgPhoto.Width;
                int imgPhotoHeight = imgPhoto.Height;
                using (Bitmap bmPhoto = new Bitmap(imgPhotoWidth, imgPhotoHeight, PixelFormat.Format24bppRgb))
                {
                    ...
                    using (Graphics gbmPhoto = Graphics.FromImage(bmPhoto))
                    {
                        ...
                        //画两次制造透明效果
                        SolidBrush semiTransBrush2 = new SolidBrush(Color.FromArgb(Convert.ToInt32(Alpha * 255), 56, 56, 56));
                        gbmPhoto.DrawString(text, font, semiTransBrush2, x + 1, y + 1);

                        SolidBrush semiTransBrush = new SolidBrush(Color.FromArgb(Convert.ToInt32(Alpha * 255), 176, 176, 176));
                        gbmPhoto.DrawString(text, font, semiTransBrush, x, y);
                        bmPhoto.Save(savePath, ImageFormat.Png);
                    }
                }
            }
        }
