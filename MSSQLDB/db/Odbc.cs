...
namespace MSSQLDB
{
    public class Odbc : DBUtils, IDataAccess
    {
        ...
        public Odbc(String DbName, String IP, String UID, String PWD)
        {
        ...
        private void CreateCommand(OdbcCommand cmd, ExecuteObject sender)
        {
            ...
            foreach (String key in schema.Keys)
            {
                if (sender.cells.Contains(key))
                {
                    ...
                    {
                        cmd.Parameters.Add(new OdbcParameter(key, sender.cells[key]));
                    }
                }
            }
            if (sender.cmdtype == CmdType.INSERT)
            ...
            if (sender.cmdtype != CmdType.INSERT)
            {
                foreach (String key in schema.Keys)
                {
                    ...
                    if (sender.terms.Contains(_key))
                    {
                        ...
                        cmd.Parameters.Add(new OdbcParameter(_key, sender.terms[key]));
                    }
                }
            }
        ...
        public Boolean ExecuteCommand(String SQL)
        {
            ...
            using (OdbcConnection connection = new OdbcConnection(this.ConnectionString))
            {
                using (OdbcCommand command = new OdbcCommand())
                {
                    ...
                    using (OdbcTransaction transaction = command.Connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (attfield != null && attfield.Length > 0)
                            {
                                ...
                                if (att.IsPrimaryKey)
                                {
                                    if (cmdtype == CmdType.UPDATE)
                                    {
                                        ...
                                        command.Parameters.Add(new OdbcParameter(att.Name, value));
                                    }
                                }
