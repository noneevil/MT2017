...
namespace MSSQLDB
{
    public class MYSql : DBUtils, IDataAccess
    {
        ...
        public MYSql(String DbName, String IP, String UID, String PWD)
        {
            ...
            //server = localhost; user id = XXXX; password = XXXX; database = XXXX
        ...
        private void CreateCommand(MySqlCommand cmd, ExecuteObject sender)
        {
            ...
            foreach (String key in schema.Keys)
            {
                if (sender.cells.Contains(key))
                {
                    ...
                    {
                        cmd.Parameters.Add(new MySqlParameter(key, sender.cells[key]));
                    }
                }
            }
            if (sender.cmdtype == CmdType.INSERT)
            ...
            if (sender.cmdtype != CmdType.INSERT)
            {
                foreach (String key in schema.Keys)
                {
                    ...
                    if (sender.terms.Contains(_key))
                    {
                        ...
                        cmd.Parameters.Add(new MySqlParameter(_key, sender.terms[key]));
                    }
                }
            }
        ...
        public DataSet ExecuteDataSet(String proceName, Hashtable Parameters)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(this.ConnectionString))
            {
                using (MySqlCommand command = new MySqlCommand())
                {
                    ...
                    {
                        command.Parameters.Add(new MySqlParameter(enumerator.Key.ToString(), enumerator.Value));
                    }
                    connection.Open();
        ...
        public Boolean ExecuteCommand(String SQL)
        {
            ...
            using (MySqlConnection connection = new MySqlConnection(this.ConnectionString))
            {
                using (MySqlCommand command = new MySqlCommand())
                {
                    ...
                    using (MySqlTransaction transaction = command.Connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (attfield != null && attfield.Length > 0)
                            {
                                ...
                                if (att.IsPrimaryKey)
                                {
                                    if (cmdtype == CmdType.UPDATE)
                                    {
                                        ...
                                        command.Parameters.Add(new MySqlParameter(att.Name, value));
                                    }
                                }
