...
namespace MSSQLDB
{
    public class Oracle : DBUtils, IDataAccess
    {
        ...
        public Oracle(String DbName, String IP, String UID, String PWD)
        {
        ...
        private void CreateCommand(OracleCommand cmd, ExecuteObject sender)
        {
            ...
            foreach (String key in schema.Keys)
            {
                if (sender.cells.Contains(key))
                {
                    ...
                    {
                        cmd.Parameters.Add(new OracleParameter(key, sender.cells[key]));
                    }
                }
            }
            if (sender.cmdtype == CmdType.INSERT)
            ...
            if (sender.cmdtype != CmdType.INSERT)
            {
                foreach (String key in schema.Keys)
                {
                    ...
                    if (sender.terms.Contains(_key))
                    {
                        ...
                        cmd.Parameters.Add(new OracleParameter(_key, sender.terms[key]));
                    }
                }
            }
        ...
        public DataSet ExecuteDataSet(String proceName, Hashtable Parameters)
        {
            ...
            using (OracleConnection connection = new OracleConnection(this.ConnectionString))
            {
                using (OracleCommand command = new OracleCommand())
                {
                    ...
                    {
                        command.Parameters.Add(new OracleParameter(enumerator.Key.ToString(), enumerator.Value));
                    }
                    connection.Open();
        ...
        public Boolean ExecuteCommand(String SQL)
        {
            ...
            using (OracleConnection connection = new OracleConnection(this.ConnectionString))
            {
                using (OracleCommand command = new OracleCommand())
                {
                    ...
                    using (OracleTransaction transaction = command.Connection.BeginTransaction())
                    {
                        try
                        {
                            ...
                            if (attfield != null && attfield.Length > 0)
                            {
                                ...
                                if (att.IsPrimaryKey)
                                {
                                    if (cmdtype == CmdType.UPDATE)
                                    {
                                        ...
                                        command.Parameters.Add(new OracleParameter(att.Name, value));
                                    }
                                }
