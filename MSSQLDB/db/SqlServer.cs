...
namespace MSSQLDB
{
    ...
    /// Sql Server 数据库
    /// </summary>
    public class SqlServer : DBUtils, IDataAccess
    {
        ...
        public SqlServer(String DbName, String IP, String UID, String PWD)
        {
        ...
        private void CreateCommand(SqlCommand cmd, ExecuteObject sender)
        {
            ...
            foreach (String key in schema.Keys)
            {
                if (sender.cells.Contains(key))
                {
                    ...
                    {
                        cmd.Parameters.Add(new SqlParameter(key, sender.cells[key]));
                    }
                }
            }
            if (sender.cmdtype == CmdType.INSERT)
            ...
            if (sender.cmdtype != CmdType.INSERT)
            {
                foreach (String key in schema.Keys)
                {
                    ...
                    if (sender.terms.Contains(_key))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter(_key, sender.terms[key]));
                    }
                }
            }
        ...
        public DataSet ExecuteDataSet(String proceName, Hashtable Parameters)
        {
            ...
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    {
                        command.Parameters.Add(new SqlParameter(enumerator.Key.ToString(), enumerator.Value));
                    }
                    connection.Open();
        ...
        public DataSet ExecuteDataSet(String proceName, Hashtable Parameters, out Hashtable outParameters)
        {
            ...
            using (SqlConnection connection = new SqlConnection(this.ConnectionString))
            {
                using (SqlCommand command = new SqlCommand())
                {
                    ...
                    while (enumerator.MoveNext())
                    {
                        SqlParameter param = new SqlParameter(enumerator.Key.ToString(), enumerator.Value);
                    ...
                    {
                        outParameters.Add(k, command.Parameters[k].Value);
                    }
                }
            }
            return set;
                            ...
                            if (attfield != null && attfield.Length > 0)
                            {
                                ...
                                if (att.IsPrimaryKey)
                                {
                                    if (cmdtype == CmdType.UPDATE)
                                    {
                                        ...
                                        command.Parameters.Add(new SqlParameter(att.Name, value));
                                    }
                                }
        ...
        {
            return ExecuteDataSet(proceName, Parameters, out outParameters).Tables[0];
        }
        #endregion
        ...
        {
            T result = Activator.CreateInstance<T>();
            using (IDataReader reader = ExecuteReader(proceName, Parameters, out outParameters))
            {
                SetValue<T>(reader, ref result);
            }
            return result;
