    ...
    <div id="tabs">
	    ...
	    <div id="tabs-2" class="hide">
            <table border="0" width="96%" align="center" cellpadding="4" cellspacing="0" style="border-collapse: collapse;">
                ...
                <tr>
                    <th>操作日志：</th>
                    <td><aspx:CheckBox ID="RecordLog" runat="server" Text="启用日志" /></td>
                </tr>
            </table>
        </div>
...
<script type="text/javascript">
    window.addEvent('domready', function () {
        ...
        $('BadKeywords').value=keywords.join(',');
        $('DataType').addEvent('change', function () {
            var val = $(this).value;
            Object.each(options,function(item,index){
                if(item.Name==val){
                    $('ConnectionString').value=item.ConnectionString;
                }
            });
        });
    });
