...
namespace WebSite.Web
{
    public partial class Books : BaseAdmin
    {
        ...
        private void BindData()
        {
            ...
            String _sql = "SELECT COUNT(a.id) FROM [t_books] AS a WHERE " + filter;

            String sql = String.Format("SELECT TOP {0} a.*,b.membername FROM [t_books] AS a LEFT OUTER JOIN [t_members] AS b ON a.memberid=b.id WHERE (a.id NOT IN (SELECT TOP {1} a.id FROM [t_books] a WHERE {2} ORDER BY a.id DESC)) AND {2} ORDER BY a.id DESC", Pager1.PageSize, Pager1.PageSize * (Pager1.CurrentPageIndex - 1), filter);
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "del":
                    ...
                    T_LogsHelper.Append("删除留言:" + title.Value, Admin.ID);
                    break;
                case "status":
                    ...
                    T_LogsHelper.Append("修改留言状态:" + title.Value, Admin.ID);
                    break;
            }
            BindData();
        }
        ...
        protected void Repeater1_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                detail.Attributes.Add("onclick", String.Format("javascript:dialogIFrame({{url:'Books_Edit.aspx?id={0}',title:'详细 - {1}',width:630, height: 450}});return false;", ID, dv["title"]));
                if (!String.IsNullOrEmpty(dv["ReContent"].ToString())) detail.Src = "images/icos/mail_open.png";
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (list.Count > 0)
            {
                ...
                {
                    case 0://禁用
                        db.ExecuteCommand(String.Format("UPDATE [t_books] SET status = 0 WHERE id IN({0})", id));
                        T_LogsHelper.Append("批量禁用留言:" + String.Join(",", titles.ToArray()), Admin.ID);
                        break;
                    case -1://删除
                        db.ExecuteCommand(String.Format("DELETE FROM [t_books] WHERE id in ({0})", id));
                        T_LogsHelper.Append("批量删除留言:" + String.Join(",", titles.ToArray()), Admin.ID);
                        break;
                    case 1://启用
                        db.ExecuteCommand(String.Format("UPDATE [t_books] SET status = 1 WHERE id IN({0})", id));
                        T_LogsHelper.Append("批量启用留言:" + String.Join(",", titles.ToArray()), Admin.ID);
                        break;
                }
            }
            BindData();
