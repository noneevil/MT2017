...
namespace WebSite.Web
{
    public partial class Comment : BaseAdmin
    {
        ...
        private void BindData()
        {
            ...
            String sql = "SELECT COUNT(a.id) FROM [t_comment] AS a WHERE " + filter;

            String strSql = String.Format("SELECT TOP {0} a.*,b.membername FROM [t_comment] AS a LEFT OUTER JOIN [t_members] AS b ON a.memberid=b.id WHERE (a.id NOT IN (SELECT TOP {1} a.id FROM [t_comment] a WHERE {2} ORDER BY a.id DESC)) AND {2} ORDER BY a.id DESC", Pager1.PageSize, Pager1.PageSize * (Pager1.CurrentPageIndex - 1), filter);
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            switch (e.CommandName)
            {
                case "del":
                    ...
                    AppendLogs("删除评论:" + name.Value, LogsAction.Delete);
                    break;
                case "audit":
                    ...
                    AppendLogs("修改评论审核:" + name.Value, LogsAction.Edit);
                    break;
            }
            BindData();
        }
        ...
        protected void Repeater1_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ImageButton btnaudit = (ImageButton)e.Item.FindControl("audit");
                HtmlInputImage btndetail = (HtmlInputImage)e.Item.FindControl("detail");

                btndetail.Attributes.Add("onclick", String.Format("javascript:dialogIFrame({{url:'Comment_Edit.aspx?id={0}',title:'详细 - {1}',width:500, height: 400}});return false;", id, dv["title"].ToString()));

                if ((Boolean)dv["audit"]) btnaudit.ImageUrl = "../skin/icos/checkbox_yes.png";
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (RepeaterItem item in Repeater1.Items)
            {
                ...
                if (chkid.Checked)
                {
                    ...
                    {
                        case 0://批量未审核
                            db.ExecuteCommand(String.Format("UPDATE [t_comment] SET audit = 0 WHERE id = {0}", chkid.Text));
                            AppendLogs("评论未通过审核:" + name.Value, LogsAction.Edit);
                            break;
                        case -1://删除
                            db.ExecuteCommand(String.Format("DELETE FROM [t_comment] WHERE id = {0}", chkid.Text));
                            AppendLogs("删除评论:" + name.Value, LogsAction.Delete);
                            break;
                        case 1://批量通过审核
                            db.ExecuteCommand(String.Format("UPDATE [t_comment] SET audit = 1 WHERE id = {0}", chkid.Text));
                            AppendLogs("评论通过审核:" + name.Value, LogsAction.Edit);
                            break;
                    }
                }
            }
