...
namespace WebSite.Controls.Connector.CommandHandlers
{
    internal class ThumbnailCommandHandler : CommandHandlerBase
    {
        ...
        public override void SendResponse(System.Web.HttpResponse response)
        {
            ...
            string thumbFileExt = System.IO.Path.GetExtension(fileName).TrimStart('.').ToLower();
            String _thumbFilePath = System.IO.Path.Combine(this.CurrentFolder.ServerPath, fileName);

            string thumbFilePath = Path.Combine(HttpRuntime.AppDomainAppPath + "UpFiles/Thumb", _thumbFilePath.GetHashCode() + "." + thumbFileExt);
            //System.IO.Path.Combine(this.CurrentFolder.ThumbsServerPath, fileName);
