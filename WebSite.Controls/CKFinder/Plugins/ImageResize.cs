	...
	public class ImageResizeCommandHandler : XmlCommandHandlerBase
	{
		...
		protected override void BuildXml()
		{
			...
			if ( resizeOriginal )
			{
				...
				WebSite.Controls.Connector.ImageTools.ResizeImage( filePath, newFilePath, newWidth, newHeight, true, Config.Current.Images.Quality );
			}
			...
			foreach ( string size in sizes )
			{
				if ( Request.Form[size] != null && Request.Form[size] == "1" )
				{
					...
					if ( WebSite.Controls.Connector.Config.Current.PluginSettings.ContainsKey( "ImageResize_" + size + "Thumb" ) )
					{
						string thumbSize = WebSite.Controls.Connector.Config.Current.PluginSettings["ImageResize_" + size + "Thumb"].ToString().Trim();
...
{
	public class ImageResize : WebSite.Controls.CKFinderPlugin
	{
		...
		public void Init( WebSite.Controls.Connector.CKFinderEvent CKFinderEvent )
		{
			CKFinderEvent.BeforeExecuteCommand += new WebSite.Controls.Connector.CKFinderEvent.Hook( this.BeforeExecuteCommand );
			CKFinderEvent.InitCommand += new WebSite.Controls.Connector.CKFinderEvent.Hook( this.InitCommand );
		}
